...
With a little bit of work, we could reduce `jsonArray` and `jsonObject` into a single `jsonStructure`, with the type:
...
Now that we've got all the individual components, let's actually implement the `json` parser we mentioned earlier:
...
Now, let's look at a fragile JSON validator. What's fragile about this? Quite a bit, as we'll see in a second, but we can use this as a model for getting type-safety out of arbitrary JSON.
